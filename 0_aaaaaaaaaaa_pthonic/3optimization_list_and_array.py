...
"""
result1 = [(i, j) for i in outer_list for j in inner_list if i * len(j) > 6]
"""
**3. 执行顺序：列表推导式的执行逻辑遵循从左到右、从外到内的顺序：
    先执行最左边的 for 循环，依次取其可迭代对象的每一个元素。
    对于外部循环的每一个元素，执行内部的 for 循环，遍历其可迭代对象的所有元素。
    在内部循环中，对当前内外循环变量的组合应用 if 条件判断。如果条件为 True，则根据列表推导式的表达式生成一个新的元素，并将其添加到结果列表中；否则跳过该组合，继续处理下一个内部循环的元素。
    完成内部循环后，继续处理外部循环的下一个元素，重复上述过程，直至所有外部循环元素遍历完毕。
    
**4. 嵌套层次：理论上，列表推导式可以包含任意数量的嵌套 for 循环，形成多层嵌套结构。每增加一层循环，就相当于增加一个维度的遍历。同时，可以有多个 if 条件语句，它们之间可以是逻辑与（and）的关系，也可以通过嵌套 if-elif-else 结构实现更复杂的逻辑。
"""
nested_lists = [
    [
        [1, 2],
...
**3. 表达力强：列表推导能够轻松处理复杂的列表生成逻辑，包括嵌套循环、多重条件筛选等。这使得开发者能够以直观、一致的方式表达复杂的列表生成操作。
"""
# 创建二维矩阵（列表的列表）
