...
class ProductReporter:
    ...
    def _outputer(self, content: str) -> None:
        self.outputer.outputer(content)
    ...
    def _output_sumerize(self) -> None:
        ...
        self._outputer(f"{sumerize_content:>{total_length}}")
    ...
    def _output_line_item(self, name: str, price: str) -> str:
        ...
        self._outputer(line_item)
        # 2 获取分割符
        spliter = self._get_line_spliter()
        self._outputer(spliter)
...
def main(outputer: Outputer):
    ...
    # outputer = PrintOutputer()
    # outputer = FileOutputer()
    # outputer = CombineOutputer()

    products = [Product(**item) for item in data]
    reporter = ProductReporter(products, outputer=outputer)
    reporter.report()
